...
export const VoiceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [availableVoices, setAvailableVoices] = useState<PersonaVoice[]>(DEFAULT_PERSONA_VOICES as any);
    ...
    try {
      ...
      if (backendVoices.length > 0) {
        ...
        const mappedVoices = backendVoices.map(voice => ({
          ...
          gender: voice.gender,
          accent: voice.accent,
          image: DEFAULT_PERSONA_VOICES.find(v => v.key === voice.key)?.image || DEFAULT_PERSONA_VOICES[0].image
        }));
