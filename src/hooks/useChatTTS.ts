...
export const useChatTTS = () => {
  ...
  const executeWithRateLimit = useCallback(async (fn: () => Promise<void>) => {
    ...
    if (timeSinceLastRequest < RATE_LIMIT_DELAY) {
      ...
      console.log(
        `⏳ Rate limiting: waiting ${waitTime}ms before next TTS request`,
      );
  ...
  const generateTTS = useCallback(
    async (text: string, messageIndex: number): Promise<string | undefined> => {
      if (!selectedVoice) {
      ...
      if (currentState?.isLoading || currentState?.audioUrl) {
        console.log(
          "🔄 TTS already in progress or exists for message",
          messageIndex,
          "state:",
          currentState,
        );
        return currentState?.audioUrl; // Return existing audioUrl if available
      }

      console.log("🚀 Starting TTS generation for message", messageIndex);
        ...
        executeWithRateLimit(async () => {
          try {
            ...
            if (result.success && result.audioUrl) {
              console.log(
                "🎯 Storing TTS result for message",
                messageIndex,
                ":",
                result.audioUrl,
              );
              ...
              console.log(
                "✅ TTS generated successfully for message",
                messageIndex,
              );
              resolve(result.audioUrl); // Return the audioUrl
            } else {
  ...
  const getTTSState = useCallback(
    (messageIndex: number) => {
      return (
        ttsStates[messageIndex] || {
          isLoading: false,
          isPlaying: false,
          audioUrl: undefined,
          error: undefined,
