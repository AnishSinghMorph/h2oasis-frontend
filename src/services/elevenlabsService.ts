...
class ElevenLabsService {
   ...
   private async getCachedConfig(allowExpired: boolean = false): Promise<AgentConfig | null> {
    try {
  ...
  private async cacheConfig(config: AgentConfig): Promise<void> {
    ...
    async getCacheInfo(): Promise<{ cached: boolean; expiresAt?: number; cachedAt?: number }> {
    try {
      const cached = await AsyncStorage.getItem(ElevenLabsService.CACHE_KEY);
